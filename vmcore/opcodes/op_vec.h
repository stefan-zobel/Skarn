...
inline void vec_new_cap(Context* ctx, Value* dst, uint32_t cap) {
...
inline void vec_new(Context* ctx, Value* dst) {
...
SKARN_NOINLINE inline void vec_grow(Context* ctx, Value* vec_slot) {
...
// on the grow path.
inline void vec_push(Context* ctx, Value* vec_slot, Value* val_slot) {
    GcObject* hdr     = GcObject::from_slots(vec_slot->asPtr());
