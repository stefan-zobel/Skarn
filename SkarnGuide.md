...
Every code block in this guide is a real program that compiles and runs, and the project's test harness checks
it. A `// => ...` comment is a line the program prints — in order, and a block that has them prints exactly
those lines; anything after two spaces and a `(` is a remark about that line. A few blocks continue an earlier
one and are run together with it; a block that shows a compile error marks each rejected line with
`// error: ...`; the network examples are type-checked but not run.
...
The standard library has a reference of its own, [The Skarn Standard Library](SkarnStdlib.md): every
function of every module, with its signature, and a worked example for most of them.
...
```rust
let flags   = 0xFF
let allOnes = 0xFFFF_FFFF_FFFF    // every bit set, in 48-bit two's complement
println(flags)                    // => 255
println(allOnes)                  // => -1
println(allOnes & 0x0F)           // => 15
println(0o755)                    // => 493
println(0b1011)                   // => 11
```

Any numeric literal may carry **`_` as a digit separator**, purely for readability — it has no effect on the
value, so `1_000_000` and `1000000` are the same literal. It works in every form (decimal, fraction, exponent,
and all three radix prefixes), which is what makes a full-width mask legible:

```rust
println(1_000_000)                // => 1000000
println(0xFFFF_FFFF_FFFF)         // => -1   (the same mask as above, readable in 16-bit groups)
println(0b1010_1010)              // => 170
println(3.141_592)                // => 3.141592
```

The one rule is that a separator must sit **between digits**: `1_`, `1_.5` and `0x_FF` are errors rather than
...
let a = 3 + 4 * 2
println(a)              // => 11    (Int)
println(10.0 / 4.0)     // => 2.5   (Double)
println(3 + 0.5)        // => 3.5   (Int promoted to Double)
println(-5)             // => -5    (unary minus: a negative literal)
println(-a)             // => -11   (and prefix negation of any expression)
```
...
`+= -= *= /= %=` and the bitwise/shift forms `&= |= ^= <<= >>= >>>=` update a place in one step. `a op= b`
means exactly `a = a op b`, so the type rules and the traps are the same ones the plain operator has:
...
flags |= 0b0011         // 15   (set bits)
flags &= 0b1010         // 10   (mask)
flags <<= 2             // 40
println("${n} ${c.hits} ${v[1]} ${flags}")   // => 2 1 10 40
```
...
- **Each form accepts exactly what its plain operator accepts.** The arithmetic five are numeric (`Int` and
  `Double`, with the same `Int`→`Double` widening at the boundary); the bitwise/shift six are `Int` only, so
  `d &= 1` on a `Double` is rejected just as `d & 1` is. `s += "text"` is rejected on purpose too: repeated
  string concatenation is quadratic, and Skarn has
  [`StringBuilder`](#building-strings-efficiently--stringbuilder) for building text. Plain `s = s + t` still
  works when you really do want the copy.
...
println(Point { x: 1, y: 2 } == Point { x: 1, y: 2 })   // => true
println(Some(1) == Some(1))                             // => true
let na: Option[Int] = None
let nb: Option[Int] = None
println(na == nb)                                       // => true   (two distinct values, so this is real
                                                        //            structural equality, not the self short-circuit;
                                                        //            a bare `None == None` has no element type, so annotate)
println(Some(1) == None)                                // => false  (None's element type comes from Some(1))
println(toVec([1, 2, 3]) == toVec([1, 2, 3]))           // => true
```
...
fn f(x: dyn Eq) -> Int { 0 }      // error: 'Eq' is a compile-time marker, not usable as a trait object
```
...
```rust
println(2 + 2 == 4)           // => true
println("abc" == "abc")       // => true   (String compares its text)
println(5 > 3 && 2 <= 2)      // => true
println("apple" < "banana")   // => true   (lexicographic, byte order)
```
...
`~x` is bitwise-not — it flips every bit, equivalent to `x ^ -1`. All of them except `~` also have a
[compound-assignment form](#compound-assignment) (`&= |= ^= <<= >>= >>>=`).
...
```rust
let label = "x" + 1 + "y" + true
println(label)                      // => x1ytrue
```
...
let piped = 5 |> twice |> plus(1)   // plus(twice(5), 1)
println("piped=" + piped)          // => piped=11
```
...
println(-7 % 3)       // => -1   (the sign of the dividend)
println(3 + 0.5)      // => 3.5
```
...
If a map literal names the same key twice, the last entry wins and the earlier one is simply gone — and the
compiler **warns**. Watch for the case where the two keys do not *look* alike: in a `Map[Double, …]` an `Int`
key is widened, so `#{1 => 7, 1.0 => 8}` names one key, not two.
...
fn repeatChar(c: Int, times: Int) -> String {
    let mut buf = bytes()       // an empty byte buffer
    let mut i = 0
    while i < times {
...
`std::string` provides the everyday helpers for searching, trimming, padding and changing case — `indexOf`,
`startsWith`, `trim`, `toUpper`, `replace`, `padStart` and more, with the `isAscii…` family for classifying a
single byte. Like the rest of Skarn's string model they work on **bytes**: indices and lengths are byte
offsets, and case conversion maps only ASCII letters. All of them, with examples, are in
[the standard library reference](SkarnStdlib.md#3-strings).
...
Any function you *write* — and any library function written in Skarn, like `sum` or `minOf` — is such a value.
The exception is the small set of **built-in functions** the compiler implements directly: `len`, `toString`,
`push`, `get`, `floor`, the math natives like `sqrt`, and their kin. They can only be **called**, not passed
around; the compiler says so, and a one-line lambda turns one into a value:

```rust fail
let names = intoIter(["a", "bb"]) |> map(len) |> collect   // error: built-in function 'len' can only be called
```

```rust
let lens = intoIter(["a", "bb", "ccc"]) |> map(fn(s: String) -> Int { len(s) }) |> collect
println(toString(lens))   // => [1, 2, 3]
```
...
counter = 10                       // warning: 'counter' is reassigned after a lambda captured it
println(peek())   // => 0   (the snapshot taken at creation, not the current 10)
```

The compiler **warns** at that reassignment, because the snapshot is far more often a surprise than an
intention. It is only advice — the program is well defined and still prints `0` — and it fires only when a
lambda bound to a name (one that outlives the statement, as `peek` does) captured a `mut` binding that is
later **rebound**. A lambda handed straight to a call is consumed on the spot and never warns.
...
That distinction is also exactly where the compiler draws the line for **writing from inside a lambda**.
Mutating the shared object is fine; rebinding the captured *name* is a **compile error**, because a capture is
a copy of the binding and the write could never be seen outside:

```rust fail
let mut n = 0
let bump = fn() -> Int { n = n + 1  n }   // error: cannot assign to 'n' inside a lambda --
                                          // it is captured by value
...
- `ordinal(value)` projects the value back to its `Int`. It returns the **discriminant**, which is the
  declaration index only when you have not pinned one — with the `Status` enum above, `ordinal(Teapot)` is
  `418`, not `3`. (Java's `ordinal()` means the position instead, but Java has no pinning, so there the two
  are always the same number.) The reverse direction, `Int` → enum, does not exist: an arbitrary integer need
  not be a valid discriminant, and a value the checker believes is exhaustively matchable but is not would
  break `match`.

```rust
enum Status : Int { Ok = 200, Created, NotFound = 404, Teapot = 418 }

println(ordinal(Ok))          // => 200
println(ordinal(Created))     // => 201   (the counter resumes from the pin)
println(ordinal(Teapot))      // => 418   (the discriminant, not the position 3)
println(Teapot)               // => Teapot   (the value itself still prints its NAME)
```
...
println(match c { Color::Red => 0, Color::Green => 1, Color::Blue => 2 })   // => 1
println(s)                                  // => Rect { w: 3, h: 4 }
```
...
variant name — they coexist, and you disambiguate with the enum:

```rust
enum Token { Number, Ident }
enum Node  { Number, List }                 // `Number` here too -- allowed

let t = Token::Number
let n = Node::Number
println(t == Token::Number && n == Node::Number)   // => true
```
...
A lowercase name in a pattern **binds** the matched value — on its own it accepts anything. To bind
*and* test the same value, see [`@`-bindings](#-bindings-match-a-shape-and-keep-the-whole-value) below.
An arm can add a `if` **guard** that must also hold:
...
A leading `|` is allowed (handy when you list alternatives vertically). An alternative that can never
match — already covered by an earlier one — is reported as an unreachable-pattern warning.

Alternatives **may bind**, as long as every one of them binds the **same names, at the same types,
with the same mutability**. That is what lets several shapes share one body:

```rust
enum Expr { Add(Int, Int), Sub(Int, Int), Mul(Int, Int) }

fn operands(e: Expr) -> Int {
    match e { Add(l, r) | Sub(l, r) | Mul(l, r) => l + r }
}
println(operands(Sub(9, 4)))   // => 13
```

The rule is what makes the arm body well-defined: whichever alternative matched, the body sees the
same names. Break it and the compiler says which alternative is out of step —

```rust fail
enum Expr { Add(Int, Int), Sub(Int, Int) }

fn operands(e: Expr) -> Int {
    match e { Add(l, r) | Sub(l, _) => l + r }   // error: this or-pattern alternative does not bind 'r'
}
```

— and likewise if the two alternatives bind `r` at different types, or one as `mut` and one not.

Alternatives are **not restricted to the top level**. Inside a constructor, a tuple, a list, a struct
field, or a map value the `|` needs no parentheses, because the closing bracket or the comma already
says where the alternation ends:

```rust
enum Colour { Red, Green, Blue }

fn warm(c: Option[Colour]) -> Int {
    match c { Some(Red | Green) => 1, Some(Blue) => 0, None => -1 }
}
println(warm(Some(Green)))   // => 1
```

A nested alternation still counts toward exhaustiveness — the arm above needs no wildcard, exactly as
if it had been written out as three separate arms. Anywhere the extent of the `|` would be unclear,
group it explicitly with `( … )`; the next section has the case that needs it.

### `@`-bindings: match a shape and keep the whole value

`name @ pattern` binds `name` to the value at that position **and** requires `pattern` to match. It is
the one pattern that both tests and binds:

```rust
fn grade(n: Int) -> String {
    match n {
        k @ 0..59   => "fail(" + toString(k) + ")",
        k @ 60..100 => "pass(" + toString(k) + ")",
        _           => "out of range"
    }
}
println(grade(42))   // => fail(42)
```

For a range, `@` is convenience — a guard (`k if k >= 0 && k <= 59`) says the same thing. What a guard
**cannot** do is take a value apart while keeping it whole, and that is where `@` earns its place:

```rust
fn keepIfBig(o: Option[Int]) -> Option[Int] {
    match o {
        whole @ Some(n) => if n > 10 { whole } else { None },
        None            => None
    }
}
println(keepIfBig(Some(50)))   // => Some(50)
```

Without `@` you would rebuild `Some(n)` or nest a second `match`.

Four things to know:

- It **nests**, and the name takes the type of *its own* position — in `Some(n @ 1..9)`, `n` is the
  `Int` inside, not the `Option[Int]`.
- `mut` works: `mut k @ 0..9` gives a mutable copy for the arm.
- It is **invisible to exhaustiveness**. `whole @ Some(n)` covers exactly what `Some(n)` covers, so a
  `None` arm is still required.
- After `@` a bare `|` would belong to the arm, not to the binding, so an alternation there must be
  **parenthesized** — this is the case the grouping form exists for:
...
`@` is rejected in `let`, `for`, and parameter positions. Those always match, so `let n @ p = e` would
bind exactly what `let n = e` binds — the compiler says so rather than accepting noise.

### Range patterns

A numeric range `lo..hi` matches any value **from `lo` to `hi`, inclusive of both ends**. Each bound is an
`Int` or `Double` **literal** or the **name of a `const`** — nothing else (see below). A character literal is
an `Int`, so `'0'..'9'` matches the digit bytes. This replaces a chain of `>=`/`<=` guards for byte and
...
A range binds nothing by itself — write `k @ 0..9` when you want the matched value too — and, because
the numeric types are unbounded, a range never makes a match exhaustive on its own: a `match` built only
from ranges still needs a `_` arm. The two forms are distinct patterns, so `1..9` and `1..<9` may appear
in the same `match` without either being flagged unreachable.

#### Named bounds

A bound may also be the **name of a `const`**, so a threshold can be declared once and shared between a
`match` and the code around it:

```rust
const MIN_AGE: Int = 18
const MAX_AGE: Int = 65

fn bracket(age: Int) -> String {
    match age {
        0..<MIN_AGE      => "minor",
        MIN_AGE..MAX_AGE => "working",
        _                => "retired"
    }
}
println(bracket(17) + " " + bracket(30) + " " + bracket(70))   // => minor working retired
```

A qualified name works too — `util::LO..9` for a `pub const` of an imported module, or a bare `PI` after
`use std::math::*`. The named form denotes exactly the same set as the value it stands for, so
`MIN_AGE..MAX_AGE` and `18..65` are the same pattern and the compiler will tell you if you write both.

That is the whole list: **a literal or a constant name**. An arbitrary expression — `1..(3+4)`, `1..n`,
`1..f()` — is rejected, and when the bounds are only known at run time the language already has a better
way to say it, the guard:

```rust
fn inBand(n: Int, lo: Int, hi: Int) -> Bool {
    match n { k if k >= lo && k <= hi => true, _ => false }
}
println("${inBand(5, 1, 9)} ${inBand(50, 1, 9)}")   // => true false
```

The guard says the same thing, reads no worse, and works for any comparison rather than just a band.
Keeping range bounds constant buys something in return: the compiler knows which set each range denotes,
so it can tell you when two arms describe the same one.

#### Character ranges

Against a `Char` scrutinee, character literals work directly:
...
println(kind('7') + " " + kind('m') + " " + kind('Q') + " " + kind('!'))   // => digit lower upper other
```

Because a character literal *is* an `Int`, `'a'..'z'` and `97..122` are the same range. This shorthand is
for literals only: a `const` bound against a `Char` scrutinee is a type error.
...
field to a name equal to the field. One rule goes with it: a struct pattern must name **every** field — there is
no `..` rest pattern, so adding a field to a struct makes the compiler point at each pattern that now needs
updating. Shorthand *binds*, and so it composes with or-patterns like any other binding, provided both
alternatives bind the same names:

```rust group=quadrant
enum Shape { A(Point), B(Point) }
fn sum(s: Shape) -> Int { match s { A(Point { x, y }) | B(Point { x, y }) => x + y } }
println(sum(B(Point { x: 3, y: 4 })))   // => 7
```
...
List patterns match **only a `List`**. An `Array` or a `Vec` has no head and rest to take apart, so a list
pattern on one is a compile error — index it instead (`len(v)`, `v[i]`, `get(v, i)`):

```rust fail
let v = toVec([1, 5])
let second = match v { [_, d] => d, _ => 0 }   // error: a list pattern matches a List, not a Vec[Int]
```
...
let mut total = 0
for (_, v) in scores {             // `_` skips the key
    total = total + v
}
println("total="  + total)             // => total=6
...
fn spush[T](mut s: Stack[T], x: T) -> () { push(s.items, x) }
fn spop[T](mut s: Stack[T]) -> Option[T] { pop(s.items) }   // Option: the stack may be empty

let mut s: Stack[Int] = newStack()
spush(s, 10)
spush(s, 20)
...
```rust group=shape
println("circle=" + area(Circle { r: 2.0 }))   // => circle=12.56636   (free-function form)
let sq = Square { s: 3.0 }
println("square=" + sq.area())                 // => square=9.0   (method form, equivalent)
```
...
The `mut` belongs to the **trait's** signature, not just the impl's: callers check it against the trait, so an
impl may not add `mut` to a receiver or parameter the trait declares without it (`fn tick(self)` in the trait
with `fn tick(mut self)` in the impl is a compile error). The reverse is allowed — an impl may leave out a `mut`
the trait declares — and callers still have to pass a `mut` binding.
...
```rust group=shape
let shape = Circle { r: 1.0 }
println(area(shape))      // => 3.14159   (free-function form)
println(shape |> area)    // => 3.14159   (pipe form)
println(shape.area())     // => 3.14159   (method form -- all three call the same trait method)
```
...
- `self`'s type may not appear in any *other* parameter. This is the load-bearing rule: two different `dyn Show`
  ...
  as trait objects.
- No method may **return `Self`**, and no method may be **generic** (have its own type parameters) — both are current
  restrictions, not fundamental ones. So `Clone` (`fn clone(self) -> Self`) is *not* object-safe: `dyn Clone` is
...
### Streaming text: `split`, `lines` and `words`

Three string sources produce a `dyn Iterator[String]`, so all of the stages and terminals above apply to text.
...
`words(s)` cuts text into whitespace-separated words. It differs from `split` in exactly one way, and that
difference is the whole reason it exists: its separator is a **run** of whitespace rather than a single
occurrence, so it never yields an empty field. Leading and trailing whitespace produce nothing, and
`words("")` is empty:

```rust
println(count(split("a  b", " ")))   // => 3   (the two spaces bracket an empty field)
println(count(words("a  b")))        // => 2   (the run of spaces is one separator)
println(count(words("   a b   ")))   // => 2   (edges contribute nothing)
```

Whitespace here means the ASCII set — space, tab, newline, vertical tab, form feed and carriage return. Like
`split` and `trim`, `words` works on **bytes**, so a byte of a multi-byte UTF-8 sequence never separates and
such characters stay inside their word.

Because all three return a lazy iterator, they compose with the rest of the toolkit — a word count is one
call per line:
...
for ln in lines(text) {
    total = total + count(words(ln))
}
println("words=" + total)   // => words=9
```
...
`pub` goes on `fn` / `struct` / `enum` / `trait` / `const`. A struct's fields and an enum's variants inherit their
item's visibility (a `pub enum`'s constructors are public). **Methods carry no `pub` of their own** either: a
method in `impl Type { … }` is visible wherever `Type` is, and a trait method wherever its trait is — so
`pub fn` inside an `impl` is an error that tells you to mark the type or trait `pub` instead. Within a single module everything is visible, so
...
and `use std::process::*` for spawning processes. A native is imported like any other item — by the glob, or by
its own name (`use std::io::readFile`) — and only that way: `import std::io`, or a `use` of some *other* name
from the module, does not bring it in.
...
Everything else that reaches outside the program lives in three opt-in modules, each described with its
examples in the standard library reference:

- [`std::env`](SkarnStdlib.md#9-stdenv-arguments-environment-clocks) — the command-line arguments (`args()`),
  environment variables (`getEnv`), and the clocks `nanoTime` / `millisTime`;
- [`std::io`](SkarnStdlib.md#8-stdio-files-and-standard-input) — whole files as `Bytes` or as text, file and
  directory queries, and standard input;
- [`std::process`](SkarnStdlib.md#10-stdprocess-running-programs) — running an external program or a shell
  command and capturing its output, and asking which platform the program runs on.
...
```rust
let mut original: Vec[Int] = vec()
push(original, 1)

let mut alias = original     // both names refer to the SAME vector
push(alias, 2)
push(alias, 3)
...
Lifting `Ord` (and `<`) to the **erasure types** would need monomorphizing an erased body per erasure
instantiation, which the compiler does not do.
...
let mut a: Vec[Int] = vec()
push(a, 1)  push(a, 2)
let mut b = a          // b and a are the SAME vector
push(b, 3)
...
```rust group=alias
let mut c = clone(a)  // a fresh vector with the same elements
push(c, 4)
...
- [The Skarn Standard Library](SkarnStdlib.md) — every function of every `std` module, with its signature,
  and a worked example for most of them.
...
- The `examples/` directory has small but complete programs — a word counter, a JSON todo list, a calculator,
  a log analyzer, Dijkstra over a generic heap and more — each checked against its exact output.
- The `demo/` directory has runnable programs — one per language feature and most standard-library
  modules, and larger ones: a multi-module path tracer (`raytracer/`), a chat server on actors and active
  sockets (`chat/`), an HTTP server with worker actors (`actor_server/`) and a JSON parser written from
  scratch. [demo/README.md](demo/README.md) lists them all.
- A few features exist in the language's design space but are intentionally not part of this guide because they
  ...
  Everything shown above works today.
